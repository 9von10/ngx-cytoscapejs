...
describe('CxService', () => {
  ...
  ])('convert with null parameters should return null - %j %j', (cxData, converters) => {
  ...
  ])('convert with inconvertible data should return null - %j %j', (cxData, converters) => {
    expect(service.convert(cxData!, converters!)).toBeNull();
  });
  ...
  test.each([
    [cx1ConvertibleInput, [CxConverter.cx2js, CxConverter.cxVizConverter], cx1ConvertedOutput],
    [cx2ConvertibleInput, [CxConverter.cxVizConverter, CxConverter.cx2js], cx2ConvertedOutput],
  ])('convert should respect converter order - %j %j', (cxData, converters, expectedOutput) => {
    expect(service.convert(cxData!, converters!)).toEqual(expectedOutput);
  });
